...
def q1():
    arquivo = open("./lista4/q1.dat","r")
    for linha in arquivo:
        numeros = linha.strip().split(';')
        num = input('Digite o Número que deseja localizar: ')
        try:
            numeros.index(num)
            print(f'O número {num} está na posição {numeros.index(num)+1} da lista')
        except:
...
#numerada.
def q2():
    arquivo = open("./lista4/q2.dat", "w")
    for _ in range(10):
        letra = input('Digite um Letra: ')
        arquivo.write(letra[0]+";")
